    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#6B6459] mt-0.5">系统操作审计与安全追溯</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-[#B8964A] hover:text-[#D8C08A] bg-[#B8964A]/8 hover:bg-[#B8964A]/12 border border-[#B8964A]/15 rounded-md transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      {/* Error */}
      {error && (
        <div className="px-3 py-2.5 rounded-lg bg-[#C04030]/10 border border-[#C04030]/20 text-[#D06050] text-xs flex items-center gap-2">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/[0.06] bg-[#1A2332]/60">
              <th className="text-left px-5 py-3 text-[10px] font-medium text-[#6B6459] uppercase tracking-[0.08em]">
              ...
              <th className="text-left px-5 py-3 text-[10px] font-medium text-[#6B6459] uppercase tracking-[0.08em]">详情</th>
              <th className="text-left px-5 py-3 text-[10px] font-medium text-[#6B6459] uppercase tracking-[0.08em]">
          ...
          <tbody>
            {loading && logs.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-12 text-center text-[#4A4540] text-xs">
            ...
            {logs.map(l => (
              <tr key={l.id} className="border-b border-white/[0.03] last:border-0 hover:bg-white/[0.02] transition-colors">
                <td className="px-5 py-3 text-[#6B6459] font-mono text-[11px] whitespace-nowrap">
                  {formatTime(l.createdAt)}
                </td>
                <td className="px-5 py-3">
                  <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-medium border ${getActionStyle(l.action)}`}>
                    {l.action}
                  </span>
                </td>
                <td className="px-5 py-3 text-[#A09888] text-xs">{l.resource}</td>
                <td className="px-5 py-3 text-[#6B6459] text-[11px] max-w-[260px] truncate" title={l.detail || undefined}>
                  {l.detail || '-'}
                </td>
                <td className="px-5 py-3 text-[#D8D2C8] text-xs">{l.operator}</td>
              </tr>
            ))}
          </tbody>
        </table>
